...
function getMenuKeyboard() {
    return {
        inline_keyboard: [
            ...
            [{ text: "📖 Indeks", callback_data: "indeks" }, { text: "ℹ️ Tentang", callback_data: "tentang" }],
            [{ text: "❓ Bantuan", callback_data: "help" }, { text: "❌ Tutup", callback_data: "tutup" }]
        ]
    };
}
...
function showIndexPage(chatId, messageId, pageNumber) {
    ...
    let headers = data[0];
    let filesPerPage = 10;
    let startIndex = (pageNumber - 1) * filesPerPage + 1;
    let endIndex = Math.min(startIndex + filesPerPage, data.length);
...
• Menu utama berisi tombol untuk:
    ...
    • <b>ℹ️ Tentang:</b> Menampilkan informasi tentang bot.
...
function simpanSheets(objectData) {
    ...
    return "File berhasil disimpan!"; // Return value tidak digunakan
}
...
function doPost(e) {
    ...
    if (update.callback_query) {
        ...
        if (callbackData === "cari") {
            // Tidak perlu melakukan apa-apa, karena tombol "Cari" sekarang mengaktifkan mode inline
    ...
    if (update.message) {
        ...
        if (userId == telegramAdminID) {
            ...
            if (message.document || message.photo || message.video || message.audio) {
                ...
                resultMessage = `File '${fileName || 'Tidak Ada Nama'}' berhasil disimpan ✅`; //Perubahan format pesan
